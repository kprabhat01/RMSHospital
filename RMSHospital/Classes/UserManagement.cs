...
namespace WindowsFormsApplication1.Classes
{
    class UserManagement
    {
        ...
        //
        public static DataTable UsersInfo;


        public static void GetUsers() {

            try {
                UsersInfo = new DataTable();
                UsersInfo = SqlHelper.ReturnRows("select id, concat(ufname,' ',ulname) as Username from users where utype=1 and deleteflag=0 order by Username");
            }
            catch(Exception ex)
            {
